...
const InscriptionList = () => {
  ...
  const { useGetAll, useRemove, useConfirm, useCancel } = useInscriptions();
  ...
  const handleDelete = async (id) => {
    if (
      window.confirm("Êtes-vous sûr de vouloir supprimer cette inscription ?")
    ) {
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="flex-1">
          <SearchBar
            ...
            placeholder="Rechercher par adhérent ou sortie..."
          />
        </div>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {paginatedInscriptions.length === 0 ? (
                ...
                paginatedInscriptions.map((inscription, index) => {
                  ...
                  const sortieName =
                    sortieMap[inscription.id_sortie] ||
