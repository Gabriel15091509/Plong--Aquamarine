...
// logique des données de formation-service par rapport au reste — voir
...
const sequelize = new Sequelize(
  ...
  process.env.DB_PASSWORD,
  {
    ...
    // explicitement son propre `schema` (voir models/Formation.js) — c'est ce
    ...
    searchPath: process.env.DB_SCHEMA || "formation",
...
const testConnection = async () => {
  try {
    ...
    console.log("[formation-service] Connexion à la base de données établie.");
    return true;
  } catch (error) {
    console.error("[formation-service] Impossible de se connecter à la base de données :", error);
    return false;
  }
};
