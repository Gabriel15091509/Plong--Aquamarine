...
const { getAdherentForUser, getTresorierIdForUser, computeStatutPaiement } = require('../utils/roleScope');
...
class AdhesionService extends BaseService {
  ...
  async create(data, user = null) {
    ...
    if (montantPaye > 0) {
      ...
      await this.notifyPayment(adhesion);
    }

    return adhesion;
  }

  async notifyPayment(adhesion) {
    try {
      const adherent = await Adherent.findByPk(adhesion.num_adherent);
      if (!adherent) return;
  ...
  async enregistrerPaiementComplementaire(id_adhesion, { montant, mode, description }, user = null) {
    ...
    const { updated, isDuplicate } = await sequelize.transaction(async (t) => {
      ...
      if (isDuplicate) return { updated: adhesion, isDuplicate: true };
      ...
      const updated = await this.adhesionRepository.update(
        id_adhesion,
