...
const { isStaff, getAdherentForUser } = require('../utils/roleScope');
...
class PalanqueeService extends BaseService {
  ...
  async getAll(user = null) {
    ...
    if (adherent) {
      return await this.palanqueeRepository.findByAdherent(adherent.num_adherent);
    }
    return await this.palanqueeRepository.findAllDetailed();
  }

  async getById(id, user = null) {
    const palanquee = await this.palanqueeRepository.findByIdDetailed(id);
    if (palanquee) await this.assertCanAccessPalanquee(palanquee, user);
    return palanquee;
  }
  ...
  async getByPlongee(id_plongee, user = null) {
    const results = await this.palanqueeRepository.findByPlongee(id_plongee);
    return await this.filterForAdherent(results, user);
  }

  async getBySortie(id_sortie, user = null) {
    const results = await this.palanqueeRepository.findBySortie(id_sortie);
    return await this.filterForAdherent(results, user);
  }

  async getByMoniteur(id_moniteur) {
    return await this.palanqueeRepository.findByMoniteur(id_moniteur);
  }
  ...
  async addMembre(id_palanquee, num_adherent, user = null) {
    this.assertStaff(user);
    ...
    const nouvelAdherent = await Adherent.findOne({ where: { num_adherent } });
    if (!nouvelAdherent) throw new Error('Adhérent non trouvé');

    const niveauxActuels = (palanquee.composers || []).map(
      (c) => c.adherent?.niveau,
    );
  ...
  async retournerMateriel(id_palanquee, retours = [], user = null) {
    this.assertStaff(user);
    ...
    const ReparationService = require('./ReparationService');
    const attributionService = new AttributionService();
    const reparationService = new ReparationService();
    ...
    for (const retour of retours) {
      ...
      if (['À réparer', 'Hors service'].includes(retour.etat_retour)) {
        await reparationService.create({
