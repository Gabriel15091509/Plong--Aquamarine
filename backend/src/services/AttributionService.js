...
const MaterielService = require('./MaterielService');
const ReparationService = require('./ReparationService');
const PaiementService = require('./PaiementService');
const { getTresorierIdForUser } = require('../utils/roleScope');
...
class AttributionService extends BaseService {
  ...
  async getByPalanquee(id_palanquee) {
    return await this.attributionRepository.findByPalanquee(id_palanquee);
  }
  ...
  async create(data) {
    const disponible = await this.materielService.checkAvailability(data.num_inventaire);
    if (!disponible) {
  ...
  async enregistrerCaution(id, { montant, mode }, user = null) {
  ...
  async restituerCaution(id) {
    ...
    const paiement = await this.paiementService.paiementRepository.findOne({
      type_paiement: "Caution",
  ...
  async traiterDeterioration(id, { description, prestataire, cout_reparation }) {
    const attribution = await this.attributionRepository.findById(id);
    if (!attribution) throw new Error("Attribution non trouvée");
    ...
    const reparation = await this.reparationService.create({
